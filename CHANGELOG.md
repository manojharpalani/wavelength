...
- **Changed: the logged-in experience is now a real app with real routes, centered on the team working agreement and your personal manual, instead of the marketing home page with a small account menu bolted on.** Signing in now lands on a new `/dashboard` — your personal manual's completion at a glance, and a card per team showing member count and working-agreement status (Not started / Draft — N of 8 / Finalized), with a one-click link straight into that team's agreement. A persistent left-nav (Dashboard / My Manual / Teams, collapsing to a drawer on mobile) replaces the copy-pasted top bar that used to repeat on every screen. Teammate browsing is richer too: the roster is now a grid of profile cards (generated initials avatar, name, role, Myers-Briggs badge) instead of a plain list, and clicking one opens a full page at `/teams/[teamId]/members/[userId]` (with its own shareable-within-the-team URL) instead of a modal. Your own manual gets the same treatment at `/manual` — a read-only profile view with an "Edit basics" disclosure for name/role/MBTI so quick fixes don't require the full wizard, plus "Edit full manual" into the wizard for everything else. The wizard itself moved to `/manual/edit?step=<key>` and stayed reachable without an account (see docs/DECISIONS.md), so trying it before signing in still works exactly as before. Every screen (manual, teams, team detail, a member's profile, the agreement with its tab) is now a real, refreshable, shareable-within-the-team URL — the "always lands back on the home page" gap from `docs/REQUIREMENTS.md` is resolved for the app's own routes. Two additive migrations (`20260909120000_team_summary_counts.sql`, `20260909120100_roster_role.sql`) enrich `get_my_teams`/`get_team_roster` so the dashboard and profile cards don't need N+1 RPC round trips.
- **Fixed: the last edit to a personal manual could be silently lost if the tab was closed right after typing.** Autosave debounces 900ms after the last keystroke before writing to Supabase; closing or backgrounding the tab inside that window dropped the pending save with no warning. Now flushed immediately on `visibilitychange`/`pagehide`. Found while investigating a report of a manual "not saving" (the manual itself was saved — see the invite-link fix below for what that report actually was — but this was a real, separate gap turned up along the way).
