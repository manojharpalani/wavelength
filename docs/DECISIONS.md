...
## 2026-09-09 — Logged-in experience rebuilt around real routes, centered on the team agreement and the personal manual

**Decision:** The entire app used to be one client component (`app/page.tsx`) with an in-memory `view` state machine (`"home" | "wizard" | "teams" | "team" | "agreement"`) — a signed-in visitor saw the same marketing home page as everyone else, just with a small account menu swapped in, and every screen was a click or two removed from the last. That's replaced with a real Next.js route tree and a persistent nav shell:

- `/` stays the marketing page for signed-out visitors; a signed-in visitor is redirected to `/dashboard`, which is now the logged-in landing page — a card for the personal manual's completion, and one card per team with member count and an agreement-status pill (Not started / Draft — N of 8 / Finalized), linking straight into that team or its agreement.
- `/manual` is a new read-only "My Manual" profile view (avatar, name, role, MBTI, the manual body), with a small "Edit basics" disclosure for name/role/MBTI so a quick fix doesn't require the full wizard. "Edit full manual" goes to `/manual/edit?step=<key>` — the same wizard as before, just with the step as a query param instead of local state.
- `/teams`, `/teams/[teamId]`, `/teams/[teamId]/agreement?tab=<tab>` port the old list/roster/agreement screens over essentially unchanged in logic, but the roster is now a grid of profile cards (avatar, name, role, MBTI badge) instead of a plain list, and a card with `has_manual` opens a real page — `/teams/[teamId]/members/[userId]` — instead of a modal, so a teammate's manual is its own shareable-within-the-team URL rather than a one-off overlay.
- `app/(app)/layout.tsx` is a route-group layout that gates every one of those routes on being signed in (redirects to `/` otherwise) and wraps them in a new persistent `AppShell` (left nav: Dashboard / My Manual / Teams, collapsing to a drawer under 760px) — replacing the copy-pasted `.home-nav` block that used to open every `render*` function.
- Auth state moved into a `lib/auth/AuthProvider` context (mounted once in `app/layout.tsx`) instead of living in the same component as everything else, so every route can read `useAuth()` without prop-drilling from a 2000-line file. `app/page.tsx`, `app/manual/edit/page.tsx`, and each `(app)` route now each own just their own slice of state via small hooks in `lib/hooks/` (`useMyManual`, `useMyTeams`, `useTeamRoster`, `useTeamMemberManual`, `useTeamActions`, `useAgreement`) — plain `useEffect`/`useState` wrappers around the same Supabase calls as before, not a new state-management dependency.

**Why:** asked directly to rebuild the logged-in experience around the team agreement and personal manual, with fast access to teams, teammates' profiles, and your own profile/manual, calling the previous nav "clunky." Real routes were the explicit, deliberate choice over just adding more `view` values (weighed directly against the smaller, lower-risk option) — it also resolves the "a refresh or a shared link always lands on the home page" gap called out in `docs/REQUIREMENTS.md`, since every one of these screens (including the agreement's tab and the wizard's step) is now a real URL that survives a refresh.

**The wizard stays reachable without an account, on purpose.** `/manual/edit` is deliberately *outside* the `(app)` route group and its auth gate — signing in has always been "encouraged, not required" to try the wizard (`renderAuthNav`'s "Sign in to save your progress" copy), and putting it behind the new dashboard's auth wall would have silently broken that trial flow. It keeps its own left-rail chrome (the step list) rather than the shared `AppShell`, since that chrome *is* the wizard's step navigation, not a generic app nav.

**The invite-link flow (`/join/[code]` → `?join=CODE`) also stays on the signed-out-reachable marketing page, not `/teams`.** A signed-out visitor following an invite link needs to see "you've been invited to join `<team>`" (via the anon-permitted `get_team_by_invite_code` RPC) before being asked to sign in — moving that banner behind the new `/teams` route's auth gate would have broken it for exactly the people it's for. `app/join/[code]/page.tsx` is unchanged; `app/page.tsx` now owns the whole preview → sign-in → auto-join → redirect-to-team sequence that used to live in the top-level component.

**Avatars are generated (initials on a hashed color), not uploaded photos** — weighed directly against adding a Supabase Storage bucket for real photo uploads, and the no-new-infra option was chosen. `components/Avatar.tsx` hashes a name/email to a hue so the same person always renders the same color, with zero storage, migration, or RLS surface added.

**Two additive migrations** (`20260909120000_team_summary_counts.sql`, `20260909120100_roster_role.sql`) enrich `get_my_teams()` (added `member_count`, `agreement_finalized_at`, `agreement_draft_count`) and `get_team_roster()` (added `role`) so the dashboard's per-team cards and the roster's profile cards don't each need N+1 RPC round trips (loading the full 4-RPC agreement or a manual per team just to render a status pill or a job title). Same drop-then-recreate pattern as `20260908130000_roster_mbti.sql`, same aliased/column-qualified membership check, tested against real local Postgres (two seeded users, a draft answer, a finalize, and a non-member membership check) before considering the schema change done — see "Testing a schema change" in `CLAUDE.md`.

**Trade-off accepted:** the marketing hero's old "Pick up where you left off" vs. "Find your wavelength" CTA copy (based on whether the wizard already had unsaved local progress) is gone for signed-out visitors — that distinction lived in component state shared between the home page and the wizard in the same file, which no longer exists once they're separate routes. Signed-out visitors now always see "Find your wavelength"; a signed-in visitor never sees this page at all (redirected to `/dashboard`), where the manual's real completion state is shown instead. Not worth reintroducing cross-route local-draft tracking (e.g. `sessionStorage`) for a copy nuance that only mattered for the anonymous, no-account path.

## 2026-09-08 — Fixed: the last edit to a personal manual could be lost if the tab closed within the autosave debounce window

**Bug:** found while investigating a report that a manual "isn't saved," which turned out to be the invite-link join bug below (the manual data itself was there in both cases — see that entry). Along the way: the autosave effect debounces 900ms after the last keystroke before writing to `personal_manuals`, and there was no flush on tab close — a person who types a final edit and immediately closes the tab (or backgrounds the app, e.g. switching apps on mobile) loses exactly that last edit, silently, with nothing in the UI to suggest it.

**Fix:** added a second effect (same sign-in-gated lifetime as the debounce effect) that listens for `visibilitychange` (fires when `document.visibilityState` becomes `"hidden"`) and `pagehide`, and immediately flushes any pending debounced save through the same upsert. A `valuesRef` mirrors the latest `values` state so the flush always writes the current draft rather than a stale closure.

**Why `visibilitychange`/`pagehide` and not `beforeunload`:** `beforeunload` is unreliable for async work — some browsers throttle it, mobile Safari often skips it on backgrounding/app-switch entirely, and it's the pattern most commonly cited as deprecated for this exact use case. `visibilitychange`→`hidden` and `pagehide` fire earlier and more consistently (including on tab switch and backgrounding, not just close), giving the upsert's fetch a real chance to complete. Not using `sendBeacon`: it can't carry the Supabase client's auth header/session the same way `supabase.from(...).upsert(...)` does, and the added complexity of hand-rolling that request wasn't worth it for a same-origin, best-effort flush.
